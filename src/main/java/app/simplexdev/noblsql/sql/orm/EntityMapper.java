...
public class EntityMapper<T> {
    ...
    public EntityMapper(final Class<T> entityClass, final Dialect dialect) {
        ...
        if (ids.isEmpty()) {
            throw new IllegalArgumentException(
                entityClass.getName() + " must have at least one field annotated with @PrimaryKey and @Column");
        }
    ...
    private Object fromSql(final Field field, final Object raw) {
        if (raw == null && field.getType().isPrimitive()) {
            return primitiveDefault(field.getType());
        }
        final TypeHandler handler = handlers.get(field);
